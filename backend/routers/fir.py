...
def create_fir_manual(fir_data: FIRCreate, db: Session = Depends(get_db)):
...
def list_firs(skip: int = 0, limit: int = 50, db: Session = Depends(get_db)):
    """List all FIRs."""
    return db.query(FIR).order_by(FIR.created_at.desc()).offset(skip).limit(limit).all()
