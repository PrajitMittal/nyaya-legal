...
export function calculateBailEligibility(sections, arrestDateStr, chargesheetFiled, chargesheetDateStr = null) {
  ...
  const hasArrestDate = !!arrestDateStr;
  let arrestDate = null;
  let daysInCustody = 0;
  ...
  if (hasArrestDate) {
    ...
    daysInCustody = Math.floor((today - arrestDate) / (1000 * 60 * 60 * 24));
  }
  ...
  const result = {
    ...
    days_in_custody: hasArrestDate ? daysInCustody : null,
  ...
  if (allBailable) {
    result.eligibility.push({
      ...
      eligible: true,
      urgency: "IMMEDIATE",
      explanation: hasArrestDate
        ? "All charged sections are bailable offenses. Bail is a MATTER OF RIGHT under Section 436 CrPC. The police station itself should have granted bail. You do not need court permission."
        : "All charged sections are bailable offenses. If arrested, bail is a MATTER OF RIGHT under Section 436 CrPC. The police station itself must grant bail. No court permission needed.",
      legal_basis: "Section 436 CrPC / Section 478 BNSS",
      action: hasArrestDate
        ? "Apply for bail at the police station. If refused, apply before the Magistrate. Bail CANNOT be denied for bailable offenses."
        : "If arrested, demand bail at the police station immediately. Bail CANNOT be denied for bailable offenses.",
      precedent: "Rasiklal v. Kishore (2009) - Bail in bailable offenses is a right, not a privilege.",
    });
  }

  // Custody-based eligibility checks only apply when there's an arrest date
  if (hasArrestDate) {
    ...
    // No arrest date — provide general info about chargesheet and custody rules
    if (!allBailable) {
      result.eligibility.push({
        type: "Non-Bailable Offense",
        eligible: false,
        urgency: "INFO",
        explanation: `Some charged sections are non-bailable. If arrested, the police cannot grant bail at the station — a court application is required. The chargesheet must be filed within ${chargesheetDeadlineDays} days of arrest, failing which default bail becomes a right.`,
        legal_basis: "Section 437 CrPC / Section 480 BNSS",
        action: "If arrested, immediately engage a lawyer to apply for bail before the Magistrate. Note the exact date and time of arrest.",
        precedent: "State of Rajasthan v. Balchand (1977) - 'The basic rule is bail, not jail.'",
      });
    }
  }
  ...
  result.summary = {
    ...
    recommendation: hasImmediate
      ...
      : hasArrestDate
        ...
        : allBailable
          ? "All sections are bailable — bail is a right if arrested."
          : "Consult a lawyer to prepare a bail strategy.",
  };
